...
本项目是一个基于Cocos2d-x框架开发的卡牌消除类游戏。采用MVC架构模式，代码结构清晰，便于维护和扩展。
...
- **代码结构**: 分层设计（Models/Views/Controllers/Managers/Services/Configs）
...
1. **分层清晰**: Models不包含业务逻辑，Views只负责显示，Controllers协调两者
2. **职责单一**: 每个类只负责一个明确的功能
3. **易于扩展**: 新增功能只需在对应层级添加代码，不影响其他模块
4. **数据驱动**: 使用配置文件和枚举定义，便于调整和扩展

## 5. 注意事项

- 新增卡牌类型时，确保图片资源路径正确
- 新增撤销类型时，确保数据模型和视图状态都能正确恢复
- 记录撤销操作时，要在操作执行前记录，确保记录的是操作前的状态
- 视图更新时要重新绑定回调函数，确保用户交互正常
